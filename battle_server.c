...
void extract_username( int size, char* username ) {
	int i;
	for ( i = 0; i < 4; i++ ) 
		username[i] = buffer[ i + 8 ] // 4 bytes per cmd_id e 4 per username_size
}
...
int main( int argc, char** argv ) {
	...
	client_t* client_i;
	int username_size, i, fdmax, port, ret, listener, len, cmd_id;
	...
	for (;;) {
		...
		for ( i = 0; i <= fdmax; i++ ) {
			if ( FD_ISSET( i, &read_fds ) {
				...
				if ( i == listener ) {
					struct new_client* = (struct client_t*) malloc( sizeof(struct client_t);
					if ( new_client == NULL ) 
						printf( "[ERRORE] Memoria insufficiente, impossibile gestire un nuovo cliente " );
					add_client( new_client ); // TODO 
					len = sizeof(new_client->addr);
					new_client->fd = accept( listener, ( struct sockaddr * ) &new_client->addr, &len );
					new_client->ingame = 0;
